...
public class OrderTest {
	...
	public void testGetProductThroughOrder() {
		...
		// when
		Product actualProduct = order.getProductsList().get(0);
	...
	public void testGetPrice() throws Exception {
		...
		// when
		BigDecimal actualProductPrice = order.getAllProductsPrice();
	...
	public void testPriceWithTaxesWithoutRoundUp() {
		...
		// then
		assertBigDecimalCompareValue(order.getAllProductPriceWithTaxes(), BigDecimal.valueOf(2.46)); // 2.44 PLN
	}
	...
	public void testPriceWithTaxesWithRoundDown() {
		...
		// then
		assertBigDecimalCompareValue(order.getAllProductPriceWithTaxes(), BigDecimal.valueOf(0.01)); // 0.01 PLN
	...
	public void testPriceWithTaxesWithRoundUp() {
		...
		// then
		assertBigDecimalCompareValue(order.getAllProductPriceWithTaxes(), BigDecimal.valueOf(0.04)); // 0.04 PLN
	...
	@Test
	public void testIsPaidWithUncommittedPayment() throws Exception {
		// given
		Order order = getOrderWithMockedProduct();
		PaymentMethod paymentMethod = mock(MoneyTransferPaymentTransaction.class);
		given(paymentMethod.commit(any(MoneyTransfer.class))).willReturn(false);
		MoneyTransfer moneyTransfer = mock(MoneyTransfer.class);
		given(moneyTransfer.isCommitted()).willReturn(false);
	...
	@Test
	public void testIsPaidWithUncommittedMoneyTransfer() throws Exception {
		// given
		Order order = getOrderWithMockedProduct();
		PaymentMethod paymentMethod = mock(MoneyTransferPaymentTransaction.class);
		given(paymentMethod.commit(any(MoneyTransfer.class))).willReturn(true);
